...
async function startLanguage(files) {
  ...
  const name='sorobuild-lsp-'+randomUUID();let child,connection,ready=false,responsive=false,closed=false,analysisError=null;
  ...
  try {
    ...
    return {close,async request(input,{signal}={}){
      ...
      const end=Math.min(Date.now()+5000,indexingDeadline);
      while(!ready && !responsive && Date.now()<end && child.exitCode===null){checkCancellation(signal);await new Promise(resolve=>setTimeout(resolve,100));}
      checkCancellation(signal);
      if(child.exitCode!==null)throw fail(502,'Rust language server could not load this workspace.');
      // Keep HTTP requests short while the session indexes in the background.
      if(!ready && !responsive && Date.now()<indexingDeadline)throw fail(503,'Indexing the Soroban SDK. Retry shortly.');
      // Quiescence is advisory: an indexed server may already answer SDK queries.
      ...
      let result;
      try {result=await deadline(connection.sendRequest(input.method,{textDocument:{uri:uriFor(input.path)},position:input.position},cancellation.token),20000,signal);}
      finally {cancellation.cancel();signal?.removeEventListener('abort',onAbort);cancellation.dispose();}
      const meaningful=result!=null && (input.method!=='textDocument/completion' || (Array.isArray(result)?result:result.items||[]).length>0);
      if(meaningful)responsive=true;
      if(!ready && !responsive)throw fail(503,'Indexing the Soroban SDK. Retry shortly.');
